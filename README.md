...
| 성능 모니터 | FPS, 렌더 시간, E2E 지연, 메모리, 스킵된 렌더 등 실시간 추적 |
...
- **커스텀 memo 비교함수**: 코인 데이터의 실제 값(`current_price`, `market_cap` 등)을 비교하여 변경 없는 행은 리렌더 스킵
- **Structural Sharing**: React Query 폴링 응답에서 데이터가 동일한 코인은 이전 객체 참조 재사용
...
- **Web Worker (파생 지표 + Filter/Sort)**: Worker 통신 오버헤드(postMessage 직렬화) < UI 블로킹 비용. Filter/Sort는 메인 스레드에서 0.3ms였으나 Worker로 이동하여 메인 스레드 점유 0
- **FastAPI 프록시**: CORS 우회 + API key 미노출 + 서버 사이드 캐싱 확장 가능
- **가상 스크롤링 vs 페이지네이션**: 사용자 경험 우선. 스크롤만으로 전체 데이터 탐색 가능
- **Structural Sharing**: 폴링마다 전체 배열 재생성 방지. 데이터 미변경 시 참조 재사용으로 불필요한 리렌더 제거
...
Confirm 시뮬레이션
- Body (선택): `{ "force_result": "confirmed" | "failed" }`
