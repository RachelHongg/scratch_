...
## 4단계: 서버 기반 시뮬레이션 + Test Mode

### 프롬프트
- "시뮬레이션은 프론트엔드가 아니라 서버 쪽으로 보내야 한다. Refresh는 상태가 아니라 초기화 액션이다."
- "Error 시뮬레이션 시 Confirm은 항상 Failed, Confirmed 시뮬레이션 시 항상 Confirmed여야 한다."
- "시뮬레이션 버튼 클릭 시 테이블 상태는 유지, Refresh에서만 초기화."
- "테스트 모드를 만들어서 네비게이션 바에서 토글로 진입. Data Scale과 Performance Monitor는 테스트 모드에서만 표시."

### AI 생성 결과
- 백엔드: `POST /api/connection/simulate`, `POST /api/connection/reset`, `GET /api/connection/state` 엔드포인트
- 백엔드: `/api/coins` 응답에 `connection_status` 필드 추가, Error 시뮬레이션 시 503 반환
- 백엔드: `/api/confirm`이 시뮬레이션 상태에 연동 (Error → 항상 failed, Confirmed → 항상 confirmed)
- 프론트엔드: 연결 상태 머신 단순화 — `FORCE`/`RETRY`/시뮬레이션 모드 제거, 서버 응답 기반으로 상태 결정
- 프론트엔드: `RESET_ALL` 액션으로 Confirm 상태 일괄 초기화
- 프론트엔드: Test Mode 토글 (네비게이션 바) — Data Scale, Performance Monitor 조건부 렌더링

### 채택
- **서버 기반 시뮬레이션**: 프론트엔드 `FORCE` 방식을 폐기하고 서버가 실제 다른 응답을 반환하는 구조로 전환. 폴링이 돌아도 시뮬레이션 상태가 유지됨
- **Confirm API 시뮬레이션 연동**: 연결 시뮬레이션 상태에 따라 Confirm 결과도 일관되게 동작 (Error → Failed, Confirmed → Confirmed)
- **Refresh = 초기화 액션**: 시뮬레이션 해제 + Confirm 상태 전체 리셋. Live/Error/Confirmed 버튼은 테이블 상태 유지
- **Test Mode 토글**: 일반 사용자에게는 깔끔한 대시보드, 테스트 시에만 Data Scale + Performance Monitor 표시

### 수정
- **연결 상태 타입 변경**: `reconnecting` 제거, `confirmed` 추가. 상태: `connecting` → `live` | `confirmed` | `error`
- **connectionReducer 단순화**: `FORCE`, `RETRY`, `CONNECT`, `DISCONNECT`, `EXIT_SIMULATION` 제거 → `FETCHING`, `CONNECTED`, `ERROR` 3개 액션으로 단순화
- **`/api/coins` 응답 구조 변경**: 배열 직접 반환 → `{ connection_status, data }` 객체 반환. 프론트엔드 `useCoins`, `structuralSharing` 함수 모두 수정
- **SimulationControls**: `onRefresh` 단일 콜백 → `onRefetch`(데이터만 갱신) + `onRefresh`(전체 초기화) 분리

### 폐기
- **프론트엔드 FORCE 시뮬레이션**: 서버 기반으로 전환하면서 폐기. 이유: 폴링의 자동 dispatch가 FORCE 상태를 덮어쓰는 버그가 있었고, 시뮬레이션 모드 플래그로 우회하는 것보다 서버가 직접 응답을 바꾸는 것이 더 현실적
- **`reconnecting` 상태**: 요구사항 재해석 후 불필요하다고 판단하여 제거. Live/Error/Confirmed 3개로 단순화
